...
def get_required_level(path: str, protected_config: dict) -> Optional[str]:
    ...
    ``protected_endpoints`` block in DB config.
...
class TokenInfo:
    ...
    # Per-token budget overrides (None = use global defaults from DB config)
...
def build_endpoint_guard(protected_config: dict):
    ...
    control based on the ``protected_endpoints`` block from DB config.
